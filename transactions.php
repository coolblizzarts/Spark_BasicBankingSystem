      ...
      <table class="table table-hover table-dark table-striped table-condensed table-bordered">
        ...
        <tbody>
          ...
          while ($rows = mysqli_fetch_assoc($query)) {
            $query1 = mysqli_query($conn, "select name from customerdetails where id = " . $rows['sender']); //Took the name of the sender of the current transaction
            $query2 = mysqli_query($conn, "select name from customerdetails where id = " . $rows['receiver']); //Took the name of the receiver of the current transaction
          ?>

            <tr style="color : white;">
              <td class="py-2"><?php echo $rows['transaction_ID']; ?></td>
              <td class="py-2"><?php print_r($query1->fetch_assoc()['name']); ?></td>
              <!--Converted object to an array using ->fetch_assoc() and got the name-->
              <td class="py-2"><?php print_r($query2->fetch_assoc()['name']); ?></td>
